...
### Session 2026-08-25 — Enrichment-panel open crash (stowed exception) *(commit `12e37c9`)*

**Scope**: single user report — *"when i pressed scan & enrich liberary button in settings page the app crashed"*, with a debug log ending `exited with code 3221226107 (0xc000027b)` and, alongside it, `[WindowsAudioDeviceHelper] Query failed … REGDB_E_CLASSNOTREG` plus `bass_fx.dll`/`bass_ape.dll` plugin-load failures.

#### New findings discovered & fixed on the go *(this session)*

| # | Severity | Location | Finding & Fix |
|---|----------|----------|---------------|
| NF-38 | 🔴 Crash class (user-reported: the "Scan & Enrich Library" button crashed the app) | `Views/SettingsPage.xaml.cs` ctor (`ConfigureEnrichmentExpandAnimation`, removed) / `EnrichmentPanelHost` | Clicking the launcher fast-failed the process with exit `0xC000027B` — a **stowed exception**, a native/COM/WinRT fast-fail that no managed `try/catch`, `Application.UnhandledException`, `AppDomain.UnhandledException` or `UnobservedTaskException` handler can catch, because the throw never crosses back into managed code (so NF-32d's diagnostics logger recorded nothing — `dumpexceptions = 0` in every minidump). WER minidump forensics across three independent crashes (08-24 22:08, 08-24 22:10, 08-25 02:57) put the fault in `Microsoft.UI.Xaml.dll` on the UI dispatcher, over a managed stack of only `Application.Start → Program.Main` (SOS `clrstack -all` on the heap-walkable dump — pure message pump, no app frame), with a stowed HRESULT chain of `CLASS_E_CLASSNOTAVAILABLE` (0x80040111, nested XAML) → `E_INVALIDARG` whose leaf module is `combase.dll` (the COM class-activation path). **Root cause**: `EnrichmentPanelHost` was the *only* element in the entire app carrying an implicit composition show/hide animation (`ElementCompositionPreview.SetImplicitShowAnimation`/`SetImplicitHideAnimation`, armed in `ConfigureEnrichmentExpandAnimation` — confirmed unique by a full-tree grep for the implicit-composition / composition-animation APIs). Creating the animation objects was already guarded and succeeded; the fatal step was **playing** it — XAML runs the show animation *natively* the instant `ToggleLibraryEnrichmentPanel_Click` flips `Visibility` to `Visible`, and on this user's provably degraded composition/COM stack (the same box logs `REGDB_E_CLASSNOTREG` for the audio device class and failing `bass_fx`/`bass_ape` plugin loads) that commit-time path activates a composition factory through `combase` that returns `CLASS_E_CLASSNOTAVAILABLE` and fast-fails. Because the play is native and never re-enters managed code, the try/catch around *creation* could not help and the effect was unguardable from managed code. **Fix**: remove the implicit animation entirely — both the `ConfigureEnrichmentExpandAnimation()` call and the method itself. The panel now shows/hides via the plain `Visibility` toggle already in `ToggleLibraryEnrichmentPanel_Click`, a pure property set that touches no composition factory. The entrance fade/slide was documented (ENR-06) as "purely visual", so no functionality is lost. Corroborated by the 08-24 22:10 sibling crash, which faulted in `CoreMessagingXP.dll` — the composition/messaging engine. |

#### Notes & deliberate trade-offs

- **Removed the fragile operation rather than wrapping it.** A stowed exception raised by native code cannot be caught from managed code, so hardening here means *eliminating* the native call on the crash path — there is nothing to guard. Since the animation was the app's sole implicit-composition usage (per grep), nothing else regresses.
- **The panel content was eliminated as the source first.** `LibraryEnrichmentPanel` uses only standard controls and the app-wide `AcrylicBrush` (which degrades to `FallbackColor` and never hard-fails); the trigger was specifically the implicit animation firing at the `Visibility` flip, not any element in the card.
- **Attribution is a strong inference, not a symbolized frame.** No cdb/WinDbg is installed and the stowed `StackTrace` pointer arrays were not captured in these minidumps, so the leaf-frame attribution rests on the decoded stowed HRESULT chain + the `combase` leaf + the unique-composition-animation grep rather than a resolved native frame. The fix is harmless if the inference is wrong — it removes only a purely-visual effect. (The concurrent `REGDB_E_CLASSNOTREG` audio flood was already fixed as NF-32a and is independent of this crash.)

**Acceptance verified**: build `dotnet build Octave.Desktop` = 0 warnings / 0 errors · `dotnet test Octave.Core.Tests` = **333/333**. Desktop-only change (no Core-test delta; suite unchanged from the NF-37 session). Fixes the sole user report: the "Scan & Enrich Library" button now expands the card without crashing.

---

### Session 2026-08-25 — Playback-bar artwork binding + Settings backend wiring + library art *(commits `559e05b`, `c09dcd9`, `b51ea0f`, `60ab316`)*
...
| NF-35 | 🟠 Dead UI (Settings → Online Scan Preferences) | `SmartLibraryEnrichmentService` scan loop | "Re-check previously failed matches" and "Re-check ambiguous matches" persisted through the VM but **no code read them**: the scan loop's prior-state check skipped only `NeverAskAgain`, so every rescan re-queried providers for *every* already-decided track regardless of the switches. The check now routes through `ShouldSkipByPriorState(status, settings)`: `NeverAskAgain` always skips; prior `NoMatch`/`Failed` skip **unless** `RecheckPreviouslyFailedMatches`; prior `NeedsReview` skips **unless** `RecheckAmbiguousMatches`; `Pending`/applied/complete states are re-evaluated as before. Only tracks with a persisted prior state are affected (a never-scanned track is never skipped), and a skipped track keeps its status and its place in the review queue. **Behaviour change**: with the toggles at their defaults (both off), a rescan now *leaves prior no-match/failed/needs-review decisions alone* instead of silently re-hitting the network for them — which is exactly what the switch labels promise. Re-checking those tracks is now the explicit opt-in it was always presented as. |
| NF-36 | 🟠 Dead UI (Settings → Caching & Networking) | `ExternalMetadataOrchestrator` / `OnlineLyricsOrchestrator` / `ExternalArtworkOrchestrator` | Three **Caching & Networking** controls persisted through the VM but **no orchestrator read them**: every `SetAsync` used a hard-coded TTL, the stale-cache fallback ran unconditionally, and "use cached data offline" was ignored. (The section's other levers — `OfflineOnlyMode` and the per-domain master toggles — were already enforced at provider level per §18/NF-earlier, so only these three were dead.) All three external-data orchestrators now take an **optional `IExternalDataSettingsService`** ctor param — injected in production (it's registered in DI *before* the orchestrators), while the `null` default keeps every `new Orchestrator(...)` test call site on the exact prior hard-coded path — and honour: **UseCachedDataOffline** — in `OfflineOnlyMode`, turning it off short-circuits every public entry to empty/null so nothing cached *or* stale is served (providers are already disabled by `OfflineOnlyMode`); **AllowStaleCacheOnProviderFailure** — gates every `allowStale:true` fallback taken after an empty provider sweep (default on ⇒ unchanged behaviour); **CacheRetentionDays** — a `CapTtl` helper caps positive-data write TTLs to the retention window (negative-result sentinels and the NF-31 hi-res upgrade memo deliberately keep their own short TTLs). **Trade-off**: at the default 30-day retention the artwork resolved-token TTL narrows 90d→30d — this changes only how often a cover is *re-resolved*; the cached image **file** itself persists on disk via `IArtworkCacheManager`, so nothing visually regresses. +5 `CacheResilienceTests` (offline cache-use withheld vs still-served, stale gate on/off, retention TTL cap + generous-retention control). |
| NF-37 | 🟢 Requested UI addition (Library → song list) | `Views/LibraryPage.xaml` + new `ViewModels/LibraryTrackItem` / `LibraryViewModel` | The song-library list had **no album art** — only a play/pause button, title, provider glyph, artist and duration. The `Track` record deliberately carries no artwork of its own (a track knows its `AlbumId`, not a cover path), so a pure-XAML binding was impossible; the Up-Next queue solves the same problem (NP-19) by wrapping each row. Mirroring that: rows are now `LibraryTrackItem` (wraps `Track`, adds an observable `ArtworkUrl`, plus immutable `Id`/`Title`/`ArtistName`/`Provider`/`DurationSeconds` passthroughs so the row template's existing `{x:Bind}`s are untouched). `LibraryViewModel` fetches album-art tokens **once per load** via `GetAllAlbumsAsync` into an `albumId→token` map and reapplies them whenever the list is re-sorted (a sort rebuilds the wrappers) — so there is no per-row DB hit and no async hydration race. A 40×40 thumbnail column is inserted **immediately right of the play/pause button** (later columns shift +1), bound through the existing `ArtworkPathConverter` (`ConverterParameter=40`); an unknown/absent album falls back to the album placeholder. The five row/button handlers unwrap `LibraryTrackItem.Track` instead of casting to `Track`. |

#### Notes & deliberate trade-offs

- The library thumbnails read each album's **already-resolved** artwork token (the same token the queue rows and album grid use); the list never itself calls the artwork orchestrator, so scrolling triggers no network or provider work.
- Re-sorting rebuilds the row wrappers, so tokens are reapplied from the cached map rather than re-queried — one `GetAllAlbumsAsync` per library load, zero per sort.
- Tracks whose album has no cover show the standard album placeholder, consistent with every other art surface.
- The Caching & Networking artwork-token TTL cap (NF-36) narrows re-resolution cadence at low retention values but never removes cached image files — the on-disk covers these thumbnails render are untouched.

**Acceptance verified**: build `dotnet build Octave.Desktop` = 0 warnings / 0 errors · `dotnet test Octave.Core.Tests` = **333/333**. NF-37 is a Desktop-only change (no Core-test delta); the earlier NF-36 caching work added the +5 that took the suite 328→333 this session. Covers all three of the user's reports: (1) NF-34 playback-bar art re-binds after a cross-monitor drag; (2) NF-35 + NF-36 back the previously-dead Online Scan and Caching & Networking toggles (Privacy/Offline, External Providers, Metadata Writes and rate-limit were verified already wired); (3) NF-37 shows album art beside the play/pause button in the song library.
